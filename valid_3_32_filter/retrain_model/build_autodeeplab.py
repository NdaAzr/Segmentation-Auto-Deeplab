...
#from operations_2 import NaiveBN, ABN
#from modeling.modules.bn import ABN
...
#        BatchNorm2d =  nn.BatchNorm2d() #ABN if args.use_ABN else NaiveBN
...
#            print("=> use ABN!")
        if args.net_arch is not None and args.cell_arch is not None:
            network_arch, cell_arch, network_path = np.load(args.network_path), np.load(args.cell_arch), np.load(args.net_arch)
        else:
             network_arch, cell_arch, network_path= get_default_arch()
        self.encoder = newModel(network_arch, cell_arch, args.num_classes, 12, args.filter_multiplier, args=args)# BatchNorm=nn.BatchNorm2d
        self.aspp = ASPP(args.filter_multiplier * args.block_multiplier * filter_param_dict[network_path[-1]],
                         256, args.num_classes, conv=nn.Conv2d)#norm=nn.BatchNorm2d
